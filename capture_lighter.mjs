/**
 * Capture lighter emoji frames via Playwright, then encode Telegram VP9 WEBM.
 * Isolated from sign-only / volumetric pipelines (own scene + frame dir + outputs).
 */
...
const FRAMES = join(ROOT, 'render_frames_lighter');
const VARIANT = join(ROOT, 'variant_lighter');
const TOTAL = Number(process.env.HL_FRAMES || 90);
const PREVIEW = process.env.HL_PREVIEW === '1';
...
async function encode(size, name, crf, br, maxBytes) {
  ...
  const common = [
    ...
    '-passlogfile', join(ROOT, 'lighter-vp9pass'),
  ];
...
async function main() {
  ...
  await page.goto(`http://127.0.0.1:${port}/scene_lighter.html?v=${Date.now()}`, {
    waitUntil: 'networkidle',
  });
  await page.waitForFunction(() => window.ready === true, null, { timeout: 60000 });

  const frames = PREVIEW ? [1, 20, 35, 50, 70, 90] : [...Array(TOTAL)].map((_, i) => i + 1);
  ...
  for (const fr of frames) {
    ...
    if (!PREVIEW && fr % 15 === 0) console.log(`  frame ${fr}/${TOTAL}`);
  }
  ...
  if (PREVIEW) {
    console.log('Preview frames written to render_frames_lighter');
    return;
  }

  await encode(512, 'hyperlinks-space-sticker-lighter.webm', 48, '200k', 256 * 1024);
  await encode(100, 'hyperlinks-space-emoji-lighter.webm', 42, '52k', 64 * 1024);
  ...
  await run(FFMPEG, [
    ...
    '-loop', '0',
    join(ROOT, 'preview_lighter.gif'),
  ]);

  copyFileSync(join(ROOT, 'hyperlinks-space-sticker-lighter.webm'), join(VARIANT, 'hyperlinks-space-sticker-lighter.webm'));
  copyFileSync(join(ROOT, 'hyperlinks-space-emoji-lighter.webm'), join(VARIANT, 'hyperlinks-space-emoji-lighter.webm'));
  console.log('Done (lighter)');
}
